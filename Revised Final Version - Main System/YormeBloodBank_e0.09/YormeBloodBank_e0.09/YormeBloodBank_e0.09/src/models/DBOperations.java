...
//    String DB_PASSWORD = "F7SKCzgqMN";
    ...
    String DELETE_KEY = "delete from activationkeys where activationID=?";
    String CHECK_KEY = "select * from activationkeys where activationID=?";
    String GET_BANKNAME = "select bloodBank_name from bloodbank where id =?";
    String GET_BLOODBANK_ID = "select id from  bloodbank where bloodBank_name = ? ";
//    
    ...
    //view all
    String UPDATE_TABLE_VIEW_ALL_BLOODPACKS = "select bp.id,bp.packType_id,bp.bloodType, bp.date_collected,bp.donor_id, bp.date_registered, bp.account_id,bp.quantity,bp.status,pt.packType, d.donorName, d.bloodType "
                        + "from bloodpack bp,packtype pt,donor d where bp.packType_id = pt.id and bp.donor_id = d.id and bp.bloodBank_id = pt.bloodBank_id and bp.bloodBank_id = d.bloodBank_id and bp.bloodBank_id=? and bp.status = ?";
